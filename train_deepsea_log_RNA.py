...
def train(human_data_filename,mouse_data_filename,shuffled_mouse_data_filename,
          human_RNA_filename,mouse_RNA_filename,shuffled_mouse_RNA_filename,
          neg_data_ratio,positive_training_data_size,num_batch,batch_size,num_features,rnaseq_range,
          optimizer,net):
    ...
    with open(human_data_filename,'r') as positive_human_file, \
            open(mouse_data_filename,'r') as positive_mouse_file, \
            open(human_data_filename, 'r') as negative_human_file, \
            open(shuffled_mouse_data_filename,'r') as negative_mouse_file, \
            gzip.open(human_RNA_filename,'rb') as positive_human_RNA, \
            gzip.open(mouse_RNA_filename,'rb') as positive_mouse_RNA, \
            gzip.open(human_RNA_filename, 'rb') as negative_human_RNA, \
            gzip.open(shuffled_mouse_RNA_filename,'rb') as negative_mouse_RNA:
...
def eval(human_data_filename,mouse_data_filename,shuffled_mouse_data_filename,human_RNA_filename,mouse_RNA_filename,shuffled_mouse_RNA_filename,num_batch,batch_size,positive_data_size,num_features,rnaseq_range,net):
...
def findLineOffsets(total_positive_training_data_size,
    ...
    offset = 0
    with open(mouse_training_data_filename, 'r') as f:
        for i in range(total_positive_training_data_size):
            line = f.readline()
            offsets[1,i] = offset
            offset += len(line)
    ...
    offset = 0
    with open(shuffled_mouse_training_data_filename, 'r') as f:
        for i in range(total_positive_training_data_size):
            line = f.readline()
            offsets[2,i] = offset
            offset += len(line)
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='python source/train_deepsea.py', description='Train a neural network')
    ...
    g1.add_argument('-A', '--human-training-data-filename', required=True, 
                    help='path to human training binary data file', type=str)
    g1.add_argument('-B', '--mouse-training-data-filename', required=True,
                    help='path to mouse positive training binary data file', type=str)
    g1.add_argument('-C', '--shuffled-mouse-training-data-filename', required=True,
                    help='path to mouse shuffled/negative training binary data file', type=str)
    g1.add_argument('-D', '--human-training-RNA-filename', required=True, 
                    help='path to human training RNA data file', type=str)
    g1.add_argument('-E', '--mouse-training-RNA-filename', required=True,
                    help='path to mouse positive training RNA data file', type=str)
    g1.add_argument('-F', '--shuffled-mouse-training-RNA-filename', required=True,
                    help='path to mouse shuffled/negative training RNA data file', type=str)

    g1_2 = parser.add_argument_group('required arguments specifying validation data')
    g1_2.add_argument('-G', '--human-validation-data-filename', required=True,
                      help='path to human validation binary data file', type=str)
    g1_2.add_argument('-H', '--mouse-validation-data-filename', required=True,
                      help='path to mouse positive validation binary data file', type=str)
    g1_2.add_argument('-I', '--shuffled-mouse-validation-data-filename', required=True,
                      help='path to mouse shuffled/negative validation binary data file', type=str)
    g1_2.add_argument('-J', '--human-validation-RNA-filename', required=True,
                      help='path to human validation binary data file', type=str)
    g1_2.add_argument('-K', '--mouse-validation-RNA-filename', required=True,
                      help='path to mouse positive validation binary data file', type=str)
    g1_2.add_argument('-L', '--shuffled-mouse-validation-RNA-filename', required=True,
                      help='path to mouse shuffled/negative validation binary data file', type=str)
    ...
    g4.add_argument('-va', '--positive-validation-data-size',
                    ...
                    default=100000)
    g4.add_argument('-hbf', '--num-human-binary-features',
                    help='number of human features in input vector (default: 8824)', type=int, default=8824)
    g4.add_argument('-mbf', '--num-mouse-binary-features',
                    help='number of mouse features in input vector (default: 3113)', type=int, default=3113)
    g4.add_argument('-hf', '--num-human-ori-features',
                    help='number of human features in input vector (default: 8824)', type=int, default=8824)
    g4.add_argument('-mf', '--num-mouse-ori-features',
                    help='number of mouse features in input vector (default: 3113)', type=int, default=3113)
    g4.add_argument('-hrf', '--num-human-rna-features',
                    help='number of human features in input vector (default: 8824)', type=int, default=8824)
    g4.add_argument('-mrf', '--num-mouse-rna-features',
                    help='number of mouse features in input vector (default: 3113)', type=int, default=3113)
    ...
    num_human_features = args.num_human_binary_features + args.num_human_rna_features
    num_mouse_features = args.num_mouse_binary_features + args.num_mouse_rna_features
    net = PseudoSiameseNet(num_human_features, num_mouse_features, num_layers, num_neurons, dropout_rate)
    ...
    for epoch in range(args.num_epoch): # loop over the training data multiple times
        ...
        net,running_loss = train(args.human_training_data_filename,
                                 ...
                                 args.human_training_RNA_filename,
                                 args.mouse_training_RNA_filename,
                                 args.shuffled_mouse_training_RNA_filename,
                                 args.neg_data_ratio,
        ...
        validation_result = eval(args.human_validation_data_filename,
                                 ...
                                 args.human_validation_RNA_filename,
                                 args.mouse_validation_RNA_filename,
                                 args.shuffled_mouse_validation_RNA_filename,
